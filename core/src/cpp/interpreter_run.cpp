...
void StaticInterpreter::run() {
    ...
    switch (*pProgramCounter++) {
        ...
        // DataMove
        case Opcode::MOVE_B:
            eOperationSize = SIZE_BYTE;
            pDstEA = decodeEffectiveAddress();
            pSrcEA = decodeEffectiveAddress();
            *((uint8*)pDstEA) = *((uint8*)pSrcEA);
            break;
        case Opcode::MOVE_W:
            eOperationSize = SIZE_WORD;
            pDstEA = decodeEffectiveAddress();
            pSrcEA = decodeEffectiveAddress();
            *((uint16*)pDstEA) = *((uint16*)pSrcEA);
            break;
        case Opcode::MOVE_L:
            eOperationSize = SIZE_LONG;
            pDstEA = decodeEffectiveAddress();
            pSrcEA = decodeEffectiveAddress();
            *((uint32*)pDstEA) = *((uint32*)pSrcEA);
            break;

        case Opcode::MOVE_Q:
            eOperationSize = SIZE_QUAD;
            pDstEA = decodeEffectiveAddress();
            pSrcEA = decodeEffectiveAddress();
            *((uint64*)pDstEA) = *((uint64*)pSrcEA);
            break;
